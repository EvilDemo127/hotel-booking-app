    ...
    <div class="rounded-xl border border-slate-700 overflow-hidden">
        <table class="w-full text-sm text-left text-gray-300">
            <thead class="text-xs uppercase bg-slate-800 border-b border-slate-700">
                <tr>
                    <th class="px-6 py-4 font-semibold">Room Id</th>
            ...
            <tbody class="divide-y divide-slate-800 bg-slate-900/50">
                @foreach ($bookings as $booking)
                    <tr class="hover:bg-slate-800/60 transition-colors duration-150">
                        <td class="px-6 py-4 font-medium text-white">{{ $booking->id }}</td>
                        ...
                        <td class="px-6 py-4 text-emerald-400 font-medium">{{ $booking->check_in }}</td>
                        <td class="px-6 py-4 text-red-400 font-medium">{{ $booking->check_out }}</td>
                        <td class="px-6 py-4 text-center">
                            ...
                            @elseif ($booking->status == 'pending')
                               <span class="px-3 py-1 bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 text-xs rounded-full font-semibold">
                                {{ $booking->status }}
                               </span>
                            @else
                               <span class="px-3 py-1 bg-red-500/10 text-red-400 border border-red-500/20 text-xs rounded-full font-semibold">
                                {{ $booking->status }}
                               </span> 
                            @endif
                        </td>
